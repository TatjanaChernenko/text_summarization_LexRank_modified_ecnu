...
Usage:
    lex_rank_en3.py (my-lex-rank) [--length=<length>] [--language=<lang>] [--stopwords=<file_path>] [--format=<format>]
    lex_rank_en3.py (my-lex-rank) [--length=<length>] [--language=<lang>] [--stopwords=<file_path>] [--format=<format>] --url=<url>
    lex_rank_en3.py (my-lex-rank) [--length=<length>] [--language=<lang>] [--stopwords=<file_path>] [--format=<format>] --file=<file_path>
    lex_rank_en3.py (my-lex-rank) [--length=<length>] [--language=<lang>] [--stopwords=<file_path>] [--format=<format>] --text=<text>
    lex_rank_en3.py --version
    lex_rank_en3.py --help
Options:
...
"""
# Example usage: python3 lex_rank_en3.py my-lex-rank --length=10 --url=http://en.wikipedia.org/wiki/Automatic_summarization
...
class LexRankSummarizer(AbstractSummarizer):
    ...
    def __call__(self, document, sentences_count,file_small):
        ...
        m1 = self.write_csv(document.sentences,file_small)  #NEW
        
        gb = stst.Classifier(stst.GradientBoostingRegression())
        model = stst.Model('S3-gb', gb)
        ...
        new_test_file = "./data/stsbenchmark/sent_"+file_small+".csv"
        #new_test_file = "./data/stsbenchmark/sent.csv"
        ...
        test_instances= stst.load_parse_data(new_test_file, parser)
        print("PARSED!")
        model.test(test_instances, new_test_file)
        print("DONE!")
        ###################################
        #matrix = self._create_matrix(sentences_words, self.threshold, tf_metrics, idf_metrics)
        matrix = self._create_en_matrix(document.sentences, self.threshold, file_small) #NEW
    ...
    def write_csv(self, sent,file_small):

        # create matrix |sentences|×|sentences| filled with zeroes
        sentences_count = len(sent)
        matrix1 = numpy.zeros((sentences_count, sentences_count))
        degrees = numpy.zeros((sentences_count, ))
        if os.name=="nt":
            with open(path_csv +"/"+"sent_"+file_small+".csv", "w") as f:
                 for row, (sentence1) in enumerate(zip(sent)):
                    for col, (sentence2) in enumerate(zip(sent)):
                        f.write("0"+"	"+"0"+"	"+"0"+"	"+"0"+"	"+"0"+"	")
                        f.write(str(sentence1[0]))
                        f.write("	")
                        f.write(str(sentence2[0]))
                        f.write("\n")
        else:
            with open(path_csv +"sent.csv", "w") as f:
                for row, (sentence1) in enumerate(zip(sent)):
                    for col, (sentence2) in enumerate(zip(sent)):
                        f.write("0"+"	"+"0"+"	"+"0"+"	"+"0"+"	"+"0"+"	")
                        f.write(str(sentence1[0]))
    ...
    def _create_matrix(self, sentences, threshold, tf_metrics, idf_metrics):
        ...
        for row, (sentence1, tf1) in enumerate(zip(sentences, tf_metrics)):
            for col, (sentence2, tf2) in enumerate(zip(sentences, tf_metrics)):
                ...
                if matrix[row, col] > threshold: #TO DO: RERANKER
                    matrix[row, col] = 1.0
                    degrees[row] += 1
                else:
                    matrix[row, col] = 0
    ...
    def _create_en_matrix(self, sentences, threshold,file_small):
        """
        Creates matrix of shape |sentences|×|sentences|.
        """
        # create matrix |sentences|×|sentences| filled with zeroes
        ...
        for row, (sentence1) in enumerate(zip(sentences)):
            for col, (sentence2) in enumerate(zip(sentences)):
                if not sentence1==[] and not sentence2==[]:
                    if not str(sentence1[0]).startswith("&") and not str(sentence2[0]).startswith("&"): 
                        #matrix[row, col] = self.en_similarity(sentence1, sentence2,"/home/tatiana/Desktop/4_Semester/A_Textzusam/Probe/STST/generate/outputs/S3-gb/sent_"+file_small+".csv")
                        matrix[row, col] = self.en_similarity(sentence1, sentence2,"./generate/outputs/S3-gb/sent_"+file_small+".csv")
                    #print("\n\nMR\n", matrix[row, col])
                        if matrix[row, col] > threshold: #RERANKER. no, threhold
                            matrix[row, col] = 1.0
                            degrees[row] += 1
                        else:
                            matrix[row, col] = 0
    ...
    @staticmethod
    def en_similarity(sen1,sen2,out_file):
...
def main(args=None):
    ...
    all_sum = []
    for sentence in summarizer(parser.document, items_count,file_small):
...
if __name__ == "__main__":
    #path = "/home/tatiana/Desktop/4_Semester/A_Textzusam/Probe/summaries_3/"
    path = r".\output\summaries_improved"
    #path_csv = "/home/tatiana/Desktop/4_Semester/A_Textzusam/Probe/STST/data/stsbenchmark/"
    path_csv = ".\data\stsbenchmark"
    exit_code, args = main()
    file_s = args["--file"]
    if file_s != None:
        file_small = file_s.split("/")[-1]

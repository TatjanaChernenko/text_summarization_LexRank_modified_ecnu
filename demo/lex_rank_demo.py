...
"""
# Example usage: python lex_rank_demo.py my-lex-rank --length=10 --url=http://en.wikipedia.org/wiki/Automatic_summarization
        ...
        for row, (sentence1, tf1) in enumerate(zip(sentences, tf_metrics)):
            for col, (sentence2, tf2) in enumerate(zip(sentences, tf_metrics)):
                if not sentence1==[] and not sentence2==[]:
                    #print("H: ", str(sentence1), "H2: ", str(sentence2))
...
if __name__ == "__main__":
    ...
    if file_s != None:
        ...
        with open('sum_'+file_small, 'w') as f:
            for el in exit_code:
                f.write(el)
                f.write(" ")
    exit(exit_code)
